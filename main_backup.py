...
# Retry Passcode Generation Code To avoid None
# def generate_passcode_with_retry(lock_id, from_time, to_time, coworker_name, max_retries=3, retry_delay=30):
#     retries=0
#     passcode = None

#     while retries< max_retries:
#         passcode = generate_passcode(lock_id, from_time, to_time, coworker_name)
...
#     # We can handle the fallback here, e.g., by notifying the user, logging the incident, or generating a default passcode.
#     return passcode
...
def get_lock_id_by_mac(lock_mac):
    app.logger.warning(f"locks found on page {lock_mac}")
    if not lock_mac:
        return None
...
def generate_passcode(lock_id, start_date, end_date, coworker_name, max_retries=3, retry_delay=10):
    ...
    while attempt <max_retries:
        try:
            ...
            if 'keyboardPwdId' in response_data:
                return passcode
            #elif response_data.get('errmsg') == 'The gateway is busy. Please try again later.':
...
def handle_request(data):
    ...
    if resource_id in single_passcode_door_ids:
        ...
        #Issue passcode for requested secondary door
        ...
        app.logger.info(f"Requested Main door Resource id {wellness_door_id}")
        ...
        # requested_door_passcode = generate_passcode_with_retry(lock_id, from_time, to_time, coworker_name)
        # if requested_door_passcode is not None:
        #     app.logger.info(f"Generated passcode for main door 2: {requested_door_passcode}")
        #     passcodes.append(requested_door_passcode)
        #     lock_macs.append(lock_mac)
        # else:
        #     app.logger.error("Unable to generate a passcode for the Main door. Please check the system.")
        ...
        app.logger.info(f"Requested Main door 2 Resource id {main_door_2_mac}")
        ...
        # Issue passcode for a main door 1
        main_door_1_mac = "C2:DA:2B:DC:32:7D"  # Replace with the actual main door MAC
        #main_door_1_mac = "E0:61:DA:79:64:45"
        main_door_1_door_id = get_lock_id_by_mac(main_door_1_mac)
        app.logger.info(f"Requested Main door 1 Resource id {main_door_1_door_id}")
        ...
        # main_door_1_passcode = generate_passcode_with_retry(main_door_1_door_id, from_time, to_time, coworker_name)
        # if main_door_1_passcode is not None:
        #     app.logger.info(f"Generated passcode for main door 1: {main_door_1_passcode}")
        #     passcodes.append(main_door_1_passcode)
        #     lock_macs.append(main_door_1_mac)
        # else:
        #     app.logger.error("Unable to generate a passcode for the Main door 1. Please check the system.")
        


    
        #  main_door_2_passcode = generate_passcode_with_retry(main_door_2_door_id, from_time, to_time, coworker_name)
        # if main_door_2_passcode is not None:
        #     app.logger.info(f"Generated passcode for main door 2: {main_door_2_passcode}")
        #     passcodes.append(main_door_2_passcode)
        #     lock_macs.append(main_door_2_mac)
        # else:
        #     app.logger.error("Unable to generate a passcode for the Main door 2. Please check the system.")
    ...
    if send_message(coworker_id, passcodes, coworker_name, lock_macs, from_time, to_time,data["ResourceName"], data["BookingNumber"]):
